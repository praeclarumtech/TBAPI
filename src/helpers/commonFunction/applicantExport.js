...
export const generateApplicantCsv = (
  applicants,
  selectedFields = null,
  ids
) => {
  const allFields = [
    {
      key: 'name.firstName',
      label: 'First Name',
      value: (row) => row.name?.firstName || '',
    },
    {
      key: 'name.middleName',
      label: 'Middle Name',
      value: (row) => row.name?.middleName || '',
    },
    {
      key: 'name.lastName',
      label: 'Last Name',
      value: (row) => row.name?.lastName || '',
    },
    {
      key: 'phone.phoneNumber',
      label: 'Phone Number',
      value: (row) => row.phone?.phoneNumber || '',
    },
    {
    ...
    },
    {
      key: 'passingYear',
      label: 'Passing Year',
      value: (row) => row.passingYear || '',
    },
    {
      key: 'currentPincode',
      label: 'Current Pincode',
      value: (row) => row.currentPincode || '',
    },
    {
      key: 'currentCity',
      label: 'Current City',
      value: (row) => row.currentCity || '',
    },
    {
      key: 'currentAddress',
      label: 'Current Address',
      value: (row) => row.currentAddress || '',
    },
    ...
    },
    {
      key: 'collegeName',
      label: 'College Name',
      value: (row) => row.collegeName || '',
    },
    { key: 'cgpa', label: 'CGPA', value: (row) => row.cgpa || '' },
    {
      key: 'permanentAddress',
      label: 'Permanent Address',
      value: (row) => row.permanentAddress || '',
    },
    {
    ...
    },
    {
      key: 'otherSkills',
      label: 'Other Skills',
      value: (row) => row.otherSkills || '',
    },
    { key: 'rating', label: 'Rating', value: (row) => row.rating || '' },
    {
      key: 'currentPkg',
      label: 'Current Package',
      value: (row) => row.currentPkg || '',
    },
    {
      key: 'expectedPkg',
      label: 'Expected Package',
      value: (row) => row.expectedPkg || '',
    },
    {
      key: 'noticePeriod',
      label: 'Notice Period',
      value: (row) => row.noticePeriod || '',
    },
    {
      key: 'negotiation',
      label: 'Negotiation',
      value: (row) => row.negotiation || '',
    },
    {
      key: 'workPreference',
      label: 'Work Preference',
      value: (row) => row.workPreference || '',
    },
    ...
    { key: 'status', label: 'Status', value: (row) => row.status || '' },
    {
      ...
      value: (row) => row.interviewStage || '',
    },
    {
      key: 'interviewMode',
      label: 'Interview Mode',
      value: (row) => row.interviewMode || '',
    },
    {
      key: 'currentCompanyDesignation',
    ...
    },
    {
      key: 'appliedRole',
      label: 'Applied Role',
      value: (row) => row.appliedRole || '',
    },
    {
      key: 'practicalUrl',
      label: 'Practical URL',
      value: (row) => row.practicalUrl || '',
    },
    {
      key: 'practicalFeedback',
    ...
    },
    {
      key: 'portfolioUrl',
      label: 'Portfolio URL',
      value: (row) => row.portfolioUrl || '',
    },
    { key: 'referral', label: 'Referral', value: (row) => row.referral || '' },
    {
      key: 'resumeUrl',
      label: 'Resume URL',
      value: (row) => row.resumeUrl || '',
    },
    {
      key: 'gitHubUrl',
      label: 'Github URL',
      value: (row) => row.gitHubUrl || '',
    },
    {
    ...
    },
    {
      key: 'linkedinUrl',
      label: 'LinkedIn URL',
      value: (row) => row.linkedinUrl || '',
    },
    {
      key: 'clientCvUrl',
      label: 'Client CV URL',
      value: (row) => row.clientCvUrl || '',
    },
    {
      key: 'clientFeedback',
      label: 'Client Feedback',
      value: (row) => row.clientFeedback || '',
    },
    {
      key: 'meta',
...
const normalizeAppliedSkills = async (collection, fieldName, appliedSkills) => {
  if (
    !appliedSkills ||
    (typeof appliedSkills !== 'string' && !Array.isArray(appliedSkills))
  ) {
    return [];
  }
  ...
  const values = (await collection.find({}, { [fieldName]: 1, _id: 0 })).map(
    (item) => item[fieldName]
  );
  ...
  return skillArray.map((skill) => {
    ...
    return (
      ...
      values.find((val) =>
        new RegExp(cleaned.split('').join('.*'), 'i').test(
...
const normalizeField = async (model, fieldName, input) => {
  ...
  const values = (await model.find({}, { [fieldName]: 1, _id: 0 }).lean()).map(
    (item) => item[fieldName]
  );
  ...
  const fuzzyMatch = values.find(
    (val) =>
      typeof val === 'string' &&
      new RegExp(cleanedInput.split('').join('.*'), 'i').test(
        val.replace(/\s+/g, '').toLowerCase()
      )
  );
...
const validateAndFillFields = async (data, userRole) => {
  const finalState = await normalizeLocationField(
    states,
    'state_name',
    data['State']
  );
  const finalCity = await normalizeLocationField(
    city,
    'city_name',
    data['Current City']
  );
  const finalSkills = await normalizeAppliedSkills(
    Skills,
    'skills',
    data['Applied Skills']
  );
  const finalRole = await normalizeAppliedRole(
    appliedRoleModel,
    data['Applied Role']
  );
  const finalDesignation = await normalizeCurrentCompanyDesignation(
    designation,
  ...
  return {
    ...
    workPreference:
      applicantEnum[data['Work Preference']?.toUpperCase()] || null,
    status:
    ...
    interviewMode: (() => {
      const mode = data['Interview Mode']?.trim();
      if (!mode || mode === '') return '';
      const upperMode = mode.toUpperCase();
      if (upperMode === 'ONLINE') return applicantEnum.ONLINE;
      if (upperMode === 'OFFLINE') return applicantEnum.OFFLINE;
      return '';
    })(),
...
export const processCsvRow = async (data, lineNumber, userRole) => {
  ...
  if (missingFields.length > 0) {
    errorMessages.push(
      `${missingFields.join(', ')} field(s) are required at line ${lineNumber}.`
    );
  }
  ...
  validatedData.currentCompanyDesignation =
    finalDesignation || 'Software Engineer';
  validatedData.appliedRole =
    finalRole || validatedData.currentCompanyDesignation;
