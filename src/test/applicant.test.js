...
//                 const applicantData = {
//                     name: { firstName: "minasaksi", middleName: "minasaksi", lastName: "minasaksi" },
...
//                 const res = await request(app)
//                     .get('/api/applicants/viewAllApplicant')
//                     .send({ totalExperience: 10, appliedSkills: ["JavaScript", "React"] })
//                 expect(res.status).to.equal(200);
//                 expect(res.body).to.have.property('success', true);
//             });
//         })
...
//                 const res = await request(app).get(`/api/applicants/viewApplicant/67c190bc5dd82c5b6c236594`);

//                 console.log("Response Message:", res.body.message);  // Log the actual message
...
//                 const res = await request(app).get(`/api/applicants/viewApplicant/67c191e94067816e3aadefdd`);
...
//                 const applicantData = {
//                     name: { firstName: "kinjal", middleName: "kinjal", lastName: "kinjal" },
//                     phone: { whatsappNumber: "9999999999", phoneNumber: "6789098765" },
...
//                     .send(applicantData);
//                 expect(res.status).to.equal(200);
//                 expect(res.body).to.include({ success: true, message: 'User is updated successfully.' });
...
//             it('should return 404 if applicant not found', async () => {
//                 const res = await request(app).delete(`/api/applicants/deleteApplicant/67aee55737c363546dd5d998`);
...
//                 const res = await request(app).delete(`/api/applicants/deleteApplicant/67aee4b6e4002db11a7cc691`);
...
//                 const res = await request(app).put(`/api/applicants/update/status/67aee56037c363546dd5d99b`)
//                     .send({ status: 'rejected' })

//                 expect(res.status).to.equal(202)
//                 expect(res.body).to.have.property('success', true)
//                 expect(res.body).to.have.property('message', 'Applicant status is updated successfully.')
//             })
