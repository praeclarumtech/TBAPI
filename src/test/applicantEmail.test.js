...
//                 .post('/api/email/applicant/sendEmail')
//                 .set('Authorization', `Bearer ${token}`)
//                 .send({ email_to: 'abc@gmail.com', email_bcc: 'uttam.praeclarumgmail.com', subject: 'internship' })
...
//                 const res = await request(app).delete('/api/email/applicant/deleteManyEmails').send({ ids: [''] })

//                 expect(res.status).to.equal(500);
//                 expect(res.body).to.have.property('success', false);
//                 expect(res.body).to.have.property('message', 'Failed to deleteMany emails.');
//             })
//         })
//     })
